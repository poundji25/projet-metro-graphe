...
public class Graph {
    ...
    public String getPath(Node start, Node arr){
        ...
        Node term = nodeA.getEdge(nodeB).getTeminus();
        int i = 1;
        ...
        if(nodeA.getName().equals(nodeB.getName())) {
            ...
            ligne = nodeA.getLine();
            term = nodeA.getEdge(nodeB).getTeminus();
            i = 2;
        }

        path.append(String.format("Prenez la ligne, %s direction %s.\n",
                ligneB(ligne), e.getTeminus().getName()));
    ...
    public String getPath(int start, int arr){
        try {
            Node nodeA = getNodes(start);
            Node nodeB = getNodes(arr);
            return getPath(nodeA, nodeB);
    ...
    public String getPath(String start, int arr){
        try {
            Node nodeA = getNodes(start);
            Node nodeB = getNodes(arr);
            return getPath(nodeA, nodeB);
    ...
    public String getPath(int start, String arr){
        try {
            Node nodeA = getNodes(start);
            Node nodeB = getNodes(arr);
            return getPath(nodeA, nodeB);
